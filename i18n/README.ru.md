<div align="center">
  ...
  **Бесплатный AI-провайдер для OpenClaw.**
  ...
  [🚀 Быстрый старт](#-quick-start) • [💡 Возможности](#-key-features) • [📖 Установка](#-setup-guide) • [🌐 Сайт](https://hxrouter.com)
</div>
...
Пакет этого репозитория приватный (`hxrouter-app`), поэтому запуск из исходников/Docker — это ожидаемый путь локальной разработки.
...
| `CLOUD_URL` | `https://hxrouter.com` | Серверный базовый URL эндпоинта облачной синхронизации |
...
| `NEXT_PUBLIC_CLOUD_URL` | `https://hxrouter.com` | Обратно совместимый/публичный облачный URL (приоритет `CLOUD_URL` для серверного рантайма) |
...
- Убедитесь, что `CLOUD_URL` указывает на ожидаемый облачный эндпоинт (например, `https://hxrouter.com`)
...
- Симптом обычно появляется на публичном облачном эндпоинте (`https://hxrouter.com/v1`) для непотоковых (non-streaming) вызовов.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Отправляет OpenAI-совместимый запрос напрямую на облачный эндпоинт (`https://hxrouter.com/v1/chat/completions`) с указанной моделью/ключом.
...
- Публичный облачный эндпоинт (`https://hxrouter.com/v1/chat/completions`):
...
- **Сайт**: [9router.com](https://hxrouter.com)
...
См. [Pull Requests](https://github.com/Huathy/HxRouter/pulls) для подробных инструкций.
