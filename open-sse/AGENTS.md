...
- `rtk/` — request token-killer. `index.js` compresses `tool_result` content in-place (OpenAI/Claude/Kiro shapes); `filters/` per-tool compressors + `autodetect.js`; `contextCompression.js` in-process context compression; `caveman.js` system-prompt injector.
