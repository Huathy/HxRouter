...
const PROFILES = {
  oauth: {
    ...
    providerFailureThreshold: envInt("HXROUTER_PROVIDER_FAILURE_OAUTH_THRESHOLD", 10),
    providerFailureWindowMs: envInt("HXROUTER_PROVIDER_FAILURE_OAUTH_WINDOW_MS", 15 * 60 * 1000),
    providerCooldownMs: envInt("HXROUTER_PROVIDER_FAILURE_OAUTH_COOLDOWN_MS", 5 * 60 * 1000),
  },
  apikey: {
    ...
    providerFailureThreshold: envInt("HXROUTER_PROVIDER_FAILURE_APIKEY_THRESHOLD", 5),
    providerFailureWindowMs: envInt("HXROUTER_PROVIDER_FAILURE_APIKEY_WINDOW_MS", 30 * 1000),
    providerCooldownMs: envInt("HXROUTER_PROVIDER_FAILURE_APIKEY_COOLDOWN_MS", 30 * 1000),
  },
  local: {
    // Local providers are either up or down; fail fast.
    providerFailureThreshold: envInt("HXROUTER_PROVIDER_FAILURE_LOCAL_THRESHOLD", 2),
    providerFailureWindowMs: envInt("HXROUTER_PROVIDER_FAILURE_LOCAL_WINDOW_MS", 5 * 60 * 1000),
    providerCooldownMs: envInt("HXROUTER_PROVIDER_FAILURE_LOCAL_COOLDOWN_MS", 60 * 1000),
  },
};
...
export function getProviderResilienceProfile(provider) {
  if (provider === "a6api" || provider === "a6api-cli") {
    return {
      providerFailureThreshold: envInt("HXROUTER_PROVIDER_FAILURE_A6API_THRESHOLD", 5),
      providerFailureWindowMs: envInt("HXROUTER_PROVIDER_FAILURE_A6API_WINDOW_MS", 30 * 1000),
