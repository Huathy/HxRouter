/**
 * `hxrouter xai video` — generate a Grok Imagine video through the local
 * HxRouter gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: hxrouter xai video --prompt "..." [options]

Generate a Grok Imagine video via your local HxRouter gateway
...
Options:
  ...
  --api-key <key>         HxRouter API key (or env NINE_ROUTER_API_KEY)
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-hxrouter-connection-id"] || create.headers["x-9router-connection-id"] || null;
